   ...
   if(req.url=='/register' && req.method=='POST'){ 
    ...
    if(req.url=='/login' && req.method=='POST'){
      ...
      req.on('end',async()=>{
        ...
        console.log("Hiii..."+neamil+passsword);
        const d1=await fs.readFile('student.json',{encoding:'utf-8'});
        arr=JSON.parse(d1);
        let status= arr.find(ele=>ele.email==email && ele.password==password);
        if(status){
            res.setHeader('Content-Type','application/jsn')
       return res.end(JSON.stringify({'msg':'sucess '}))
      }
      else{
        res.setHeader('Content-Type','application/jsn')
       return res.end(JSON.stringify({'msg':'invalid user,login with correct credentials '}))
      }
